<template>
  <div id="waifu">
    ...
    <div id="waifu-tool">
      <live2dItem :class-name="className" @itemClick="itemClick" @itemEnter="itemEnter" />
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    cuteMode() {
      return this.$store.getters.settings.app.cute
    }
  },
  created() {
    this.getHours()
  ...
  methods: {
    ...
    loadWidget(config) {
      ...
      let userActionTimer
      const messageArray = ['好久不见，日子过得好快呢……',
        '大坏蛋！你都多久没理人家了呀，嘤嘤嘤～',
        '嗨～快来逗我玩吧！',
        '拿小拳拳锤你胸口！',
      ...
      this.loadModelList()
      // this.initModel()
      this.loadModel(this.cuteMode.model, this.cuteMode.textures, '你好呀')
    },
    loadlimit() {
      if (screen.width >= 768) {
        this.loadWidget({
